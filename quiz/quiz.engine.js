// =======================================================
// StudentHub — AI Quiz Engine (PRODUCTION HARDENED)
// OpenAI only • Render safe • No Ollama • No local deps
// =======================================================
...
const openai = new OpenAI({
  ...
  timeout: 60_000 // ⭐ prevents hanging requests
});
...
function normalizeQuiz(aiData) {
  ...
  for (const q of aiData) {
    ...
    if (correctIndex === -1 && q.answer) {
      ...
      correctIndex = q.options.findIndex(opt => normalizeText(opt) === ans);
    }
  ...
  if (normalized.length < MIN_QUESTIONS) {
    throw new Error(`Only ${normalized.length} valid questions generated`);
  }
...
async function callAI(prompt) {
  try {
    ...
    if (err.status === 429) {
      throw new Error("AI quota exceeded. Please upgrade billing or try later.");
    }
...
async function generateQuiz({ topic, language }) {
  ...
  switch (topic) {
    ...
    case "programming":
      if (!language)
        throw new Error("Programming language required");
